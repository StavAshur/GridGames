...
def all_potential_neighbors(node, size = 1000):
    ...
    idk = []
    for i in range(len(node)):
        idk.append([p for p in adjacent(node[i]) if p[0] < size and p[1] < size and p[0] >= 0 and p[1] >= 0 and p not in node])
        idk[-1].append(node[i])
        idk[-1] = tuple(idk[-1])
...
def n_before_for_exec(n):
    output = "["
    p = inflect.engine()
    #[output.join(p.number_to_words(i)) for i in range(n)]
    for i in range(n):
        #output += f"({p.number_to_words(i).strip().replace(" ", "_")}_i, {p.number_to_words(i).strip().replace(" ", "_")}_j), "
        output += f"{p.number_to_words(i).strip().replace(" ", "_")}, "
    output += "]"
    return output
...
def main():
    ...
    for i in range(0, numAgents*floor(size/numAgents), floor(size/numAgents)):
        ...
        colors.append(random.choice([c for c in ["red", "black", "green", "blue", "goldenrod"] if c not in colors]))
...
#{"    "*len(agents)*2}    nodes_for_adding.append({inner_string})"""
    ...
    for i in range(len(agents)):
        string = f"""{string}
{"    "*i}for {p.number_to_words(i).strip().replace(" ", "_")} in pos_range_without_all({size}, {size}, {n_before_for_exec(i)} + {walls}):"""
    string = f"""{string}
{"    "*len(agents)}nodes_for_adding.append({inner_string})"""
    ...
    add_edges_to_graph_from(g, edges_for_adding)
    print("Edges added!")
    ...
    while True:
        ...
        for wall in walls:
            thing = pygame.surface.Surface((tileSize, tileSize))
            thing.fill("black")
            screen.blit(thing, (wall[0]*tileSize, wall[1]*tileSize))
